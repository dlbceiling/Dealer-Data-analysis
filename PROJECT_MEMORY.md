...
```text
C:\Users\Administrator\Desktop\扬中孙荣华.xlsx
```
...
- `7mm大板/9mm大板` 中尾缀带 `（修边）` 的整板数据，已改为按 `商品名称 + 规格` 匹配成本。
- `D-FW48G (修边)` 已按 `D-FW48G-04 (修边)` 处理。
- `1220×3000`、`1220×3600` 在成本表无单独规格时，已回退到 `1220×2440` 整板成本。
- `D-FW06G-KY-9 (不修边)` 多条成本记录时，已改为取单㎡成本更高的一条。
- 三段式尺寸 `7×1220×xxxx`、`9×1220×xxxx` 当前已确认只取后两段面积，不会把厚度乘进去。
...
- `D-FW06G-KY-9 (不修边)` 若存在多条不同成本记录，按单㎡成本更高的一条作为真实成本，不再按普通冲突处理。
...
2. 将大板别名映射和整板规格兜底规则从代码移到配置文件，方便业务维护。
3. 单独梳理 `实芯板` 分类数据，决定是否纳入大板面积/单㎡成本口径。
4. 增加“原始流水”页签，方便从商品汇总追溯到原始记录。
5. 增加成本缺失清单的界面入口，而不仅是导出。
6. 更新 `README.md`，把最新成本和毛利规则写进去。
7. 重新打包 exe，并在另一台 Windows 电脑验证运行。
